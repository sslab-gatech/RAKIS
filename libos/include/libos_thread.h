...
struct libos_rt_signal_queue {
    uint64_t put_idx;
    uint64_t get_idx;
    struct libos_signal* queue[MAX_SIGNAL_LOG];
};
 ...
 */
struct libos_signal_queue {
    __sigset_t pending_mask;
    struct libos_signal standard_signals[SIGRTMIN - 1];
...
struct libos_thread {
     ...
     */
    struct libos_signal forced_signal;
    ...
    struct libos_pollable_event pollable_event;
...
struct libos_thread_queue {
    struct libos_thread_queue* next;
...
static inline bool is_internal(struct libos_thread* thread) {
    return thread->tid == 0;
}
...
static inline struct libos_thread* get_cur_thread(void) {
    return LIBOS_TCB_GET(tp);
}
...
static inline void set_cur_thread(struct libos_thread* thread) {
    assert(thread);
...
static inline int add_thread_to_queue(struct wake_queue_head* queue, struct libos_thread* thread) {
...
static inline void wake_queue(struct wake_queue_head* queue) {
    ...
    while (qnode != WAKE_QUEUE_TAIL) {
        struct libos_thread* thread = container_of(qnode, struct libos_thread, wake_queue);
...
static inline struct libos_handle_map* get_thread_handle_map(struct libos_thread* thread) {
    return thread ? thread->handle_map : get_cur_thread()->handle_map;
}

static inline void set_handle_map(struct libos_thread* thread, struct libos_handle_map* map) {
    get_handle_map(map);
